...
export default class Contract {
    constructor(network, callback) {
        ...
        this.airlines = [];
        this.airlinesNamed = [];
        this.airlinesNamed.push("Wright Bros Bicycles"); //Set name of first one
        this.passengers = [];
    ...
    async registerNewAirline(name, address, callback) {
        ...
        //console.log("self.firstAirline", self.firstAirline)
        //console.log("registerNewAirline payload", payload)

        await self.flightSuretyApp.methods
            .registerAirline(payload.addr, payload.name)
            .send({ from: self.firstAirline, "gas": 4712388, "gasPrice": 100000000000}, (error, result) => {
                //console.log(error, result)
                //callback(error, payload);
            });

        await self.flightSuretyApp.methods.getAirlineIsRegistered(payload.addr)
        ...
        /*self.flightSuretyApp.methods
            .getAirlineByIndex(0)
            .call({ from: self.firstAirline, "gas": 4712388, "gasPrice": 100000000000}, (error, result) => {
                console.log("Airlineby Index:",result)
                callback(error, payload);
            });*/
    }
    ...
    fundNewAirline(name, address, callback) {
        ...
        self.flightSuretyApp.methods
            .payFunding()
            .send({ from: payload.addr,
                value: payload.amount,
                "gas": 4712388, "gasPrice": 100000000000}, (error, result) => {
                //console.log(error, result)
                callback(error, payload);
            });
    }
    ...
    async voteNewAirline(nonRegisteredAirline, registeredAirline, callback) {
        ...
        await self.flightSuretyApp.methods
            ...
            .send({ from: payload.registeredAirline,
                "gas": 4712388, "gasPrice": 100000000000}, (error, result) => {
                //console.log(error, result)
            });
