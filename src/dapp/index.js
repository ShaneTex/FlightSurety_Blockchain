...
*/
        ...
        DOM.elid('airline-address').addEventListener('click', () => {
        ...
        DOM.elid('submit-airline').addEventListener('click', () => {
            ...
            //push new potential airline is as a voting entity
            // Write transaction
            contract.registerNewAirline(airlineName, airlineAddress, (error, result) => {
                //Generate the array for info
                if(result){
                    const select = DOM.elid('registered-address');
                    const option = DOM.option();
                    option.text = airlineName, option.value = airlineAddress;
                    select.add(option);
                } else {
                    const select = DOM.elid('vote-address');
                    const option = DOM.option();
                    option.text = airlineName, option.value = airlineAddress;
                    select.add(option);
                }

                display('register-wrapper','New Airline', 'Created Airline', [ { label: 'Success:', error: error, value: result} ]);
            });
        })
        ...
        DOM.elid('vote-airline').addEventListener('click', () => {
            ...
            contract.voteNewAirline(voteAddress, registeredAddress, (error, result) => {
                ...
                display('register-wrapper','New Airline', 'Vote Airline', [ { label: 'Success:', error: error, value: result} ]);
            });
        })
        ...
        DOM.elid('fund-airline').addEventListener('click', () => {
            ...
            contract.fundNewAirline(airlineName, airlineAddress, (error, result) => {
                ...
                display('register-wrapper','New Airline', 'Funded Airline', [ { label: 'Name:', error: error, value: result.name} ]);
            });
        })
...
*/
        ...
        DOM.elid('submit-flight').addEventListener('click', () => {
            ...
            contract.registerFlight(flightNumber, flight.address, (error, result) => {
                ...
                display('insurance-wrapper','Flight Register', 'Registered', [ { label: 'Name:', error: error, value: result.flight} ]);
            });
        })
        ...
        DOM.elid('submit-insurance').addEventListener('click', () => {
            ...
            if(etherAmount > 0 && etherAmount <= 1.0 && passenger != ""){
                contract.buyInsurance(flightNumber, flight.address, passenger, etherAmount, (error, result) => {
                    display('insurance-wrapper','Flight Insurance', 'Purchased', [ { label: 'Name:', error: error, value: result.flight} ]);
                });
            } else {
        ...
        DOM.elid('submit-claim').addEventListener('click', () => {
            ...
            contract.checkFlightInsurance(flightNumber, flight.address, passenger, (error, result) => {
                display('claim-wrapper','Flight Insurance', 'Available?', [ { label: 'Name:', error: error, value: result} ]);
            });
        })
        ...
        DOM.elid('submit-credits').addEventListener('click', () => {
            ...
            contract.checkForCredits(flightNumber, flight.address, passenger, (error, result) => {
                display('claim-wrapper','Flight Insurance Credits', 'Available?', [ { label: 'Name:', error: error, value: result} ]);
            });
        })
